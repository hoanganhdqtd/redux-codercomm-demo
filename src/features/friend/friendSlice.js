...
const slice = createSlice({
  ...
  reducers: {
    ...
    getUsersSuccess(state, action) {
      ...
      // action.payload or response.data
      // {count, totalPages, users: [{user}, ...]}
    ...
    getUserSentRequestsSuccess(state, action) {
      state.isLoading = false;
      state.error = null;
      const { users, count, totalPages } = action.payload;
      users.forEach((user) => (state.usersById[user._id] = user));
...
// response.data:
// { user._id: {_id, name, email, friendship, ...}, ...}
// friendship: {_id, from: user._id, to: otherUser._id,
...
// friendRequest: {from: userId, to: targetUserId, status}
...
export const getUserSentRequests =
  ...
  async (dispatch) => {
    ...
    try {
      ...
      dispatch(slice.actions.getUserSentRequestsSuccess(response.data));
    } catch (error) {
      dispatch(slice.actions.hasError(error.message));
      toast.error(error.message);
    }
